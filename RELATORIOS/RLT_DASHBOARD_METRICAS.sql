...
SELECT 
    ...
    CASE 
        ...
        WHEN COUNT(DISTINCT pr.id_propriedades) >= 20 THEN 'BOM'
...
SELECT 
    ...
    'alertas',
    CASE 
        WHEN COUNT(a.id_alerta) = 0 THEN 'EXCELENTE'
        WHEN COUNT(a.id_alerta) <= 2 THEN 'BOM'
        WHEN COUNT(a.id_alerta) <= 5 THEN 'ATENÇÃO'
        ELSE 'CRÍTICO'
    END
FROM GS_WW_ALERTA a
...
SELECT 
    ...
    CASE 
        ...
        WHEN (COUNT(CASE WHEN nd.nivel_numerico >= 4 THEN 1 END) * 100.0 / COUNT(*)) <= 50 THEN 'ATENÇÃO'
        ELSE 'CRÍTICO'
    END
FROM GS_WW_PROPRIEDADE_RURAL pr
JOIN GS_WW_NIVEL_DEGRADACAO_SOLO nd ON pr.id_nivel_degradacao = nd.id_nivel_degradacao
...
ORDER BY 
    categoria,
    CASE 
        WHEN status_metrica = 'CRÍTICO' THEN 1
        WHEN status_metrica = 'ATENÇÃO' THEN 2
        WHEN status_metrica = 'REGULAR' THEN 3
        WHEN status_metrica = 'BOM' THEN 4
        ELSE 5
    END;